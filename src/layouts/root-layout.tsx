    ...
    <div className="min-h-screen bg-white text-zinc-900 flex flex-col font-sans selection:bg-zinc-100 inter-class">
      ...
      <header className="sticky top-0 z-50 h-16 border-b border-zinc-100 bg-white/80 backdrop-blur-md">
        <nav className="h-full max-w-6xl mx-auto px-6 flex items-center justify-between">
          <Link to="/" className="font-semibold tracking-tighter text-xl text-zinc-900">
            B1 <span className="text-zinc-400 font-normal">Buddy</span>
          </Link>
          
          {/* Fixed Nav Items: No red, clean spacing, responsive hidden on mobile if needed */}
          <div className="hidden sm:flex items-center gap-8 text-sm font-medium text-zinc-500">
            <Link to="/lesen" className="hover:text-zinc-900 transition-colors">Lesen</Link>
            <Link to="/horen" className="hover:text-zinc-900 transition-colors">Hören</Link>
            <Link to="/vocabulary" className="hover:text-zinc-900 transition-colors">Vokabeln</Link>
            <Link to="/profile" className="px-4 py-1.5 rounded-full border border-zinc-200 text-zinc-900 hover:bg-zinc-50 transition-all">
              Profil
            </Link>
          </div>
